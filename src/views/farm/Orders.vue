      ...
      <div v-for="o in list" :key="o.id" class="card" @click="open(o.id)">
        ...
        <div class="row" style="margin-top: 8px; gap: 12px;">
          ...
          <div style="flex: 1;">
            ...
            <div class="muted">采购商：{{ o.buyer?.name }}</div>
            <div class="row-between" style="margin-top: 4px;">
              <span class="price">¥{{ o.final_price }}</span>
              <span class="muted">{{ format(o.created_at) }}</span>
            </div>
          </div>
        </div>
      </div>
...
function format(t) { return dayjs(t).format('MM-DD HH:mm'); }
...
function open(id) { router.push(`/farm/order/${id}`); }
